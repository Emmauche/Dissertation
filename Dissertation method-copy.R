...
library(rtweet)
tweets120s <-stream_tweets(" ", time = 1200)
head(tweets120s)
...
head(tweets_covid)
# Next we view the components of these tweets
names(tweets_covid)
...
sct_name <- table(tweets_covid$screen_name)
# Displaying the result tweets and screen name
head(sct_name)
...
sct_follower <- lookup_users(c("trump","Christiano","adele"))
sct_trends <- lookup_users(c("COVID","COVID-19","corona virus"))
sct_trends
# creating a dataframe for trends
trend_df <- sct_trends[,c("screen_name","followers_count")]
trend_df
#next we create a dataframe for the screen name and followers count
user_df <- sct_follower[,c("screen_name", "followers_count")]
...
rtwt <- sct_follower[,c("text","retweet_count")]
rtwt
...
dir.create("dissertationcsv")
# writing the contents of letters into the created directory and creating new csv files
write.csv(user_df, "dissertationcsv/l1.csv", row.names = FALSE)
write.csv(trend_df, "dissertationcsv/l2.csv", row.names = FALSE)
# combining both csv files
do.call("rbind", lapply(dir("dissertationcsv", full.names = TRUE), read.csv))
library(sparklyr)
sc <- spark_connect(master = "local")

spark_read_csv(sc, "dissertationcsv/")
